...
const hasValidDate = (iso: string | null) =>
  Boolean(iso && Number.isFinite(new Date(iso).getTime()));
...
function EvidenceBadge({
  ...
  label: string;
  state: "SUMMARY READY" | "NO SUMMARY" | "INCOMPLETE" | "LINKED" | "NOT LINKED";
  icon: typeof Link2;
}) {
  const positive = state === "SUMMARY READY" || state === "LINKED";
  const unknown = state === "INCOMPLETE";
  return (
    ...
    <div>
      ...
      <ol className="mt-3 grid gap-0 sm:grid-cols-5">
        {stages.map(([label, value], index) => {
          const complete = hasValidDate(value);
          const nextComplete = hasValidDate(stages[index + 1]?.[1] ?? null);
          return (
            <li
              key={label}
              ...
              {index < stages.length - 1 && (
                <span
                  ...
                  className={cn(
                    ...
                    complete && nextComplete ? "bg-primary/70" : "bg-border",
                  )}
                />
              )}
              <span
    ...
    <article className="overflow-hidden rounded-lg border border-border bg-surface shadow-panel">
      <div className="p-4 sm:p-5">
        <header className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            <Badge variant="outline" className="shrink-0">
              {call.operationalState}
            </Badge>
            <StatusBadge status={item.interactionStatus} />
          </div>
        </header>

        <div className="mt-4 grid gap-x-4 gap-y-3 border-y border-border/70 py-3 text-sm sm:grid-cols-5">
          ...
          <div>
            ...
            <p className="font-medium capitalize">{item.direction}</p>
          </div>
          <div>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
          ...
          />
          <EvidenceBadge label="Call memory" state={call.memoryEvidence} icon={MemoryStick} />
        </div>
        <p className="mt-2 text-[10px] text-muted-foreground">
          Summary ready confirms call-side evidence only; it does not claim WhatsApp persistence.
        </p>
