 ...
 *      encoder that works in the published Worker runtime, which forbids
 *      compiling WebAssembly from bytes.
...
export async function encodeVoiceNotePcm(
  ...
  if (gateway) {
    ...
    if (env["NODE_ENV"] === "production") {
      return { ok: false, reason: encoded.reason, source: "native_gateway" };
    }
  }
