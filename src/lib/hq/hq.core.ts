...
export type HqCallOperationalState =
  "RECEIVED" | "NEGOTIATING" | "MEDIA_READY" | "ANSWERED" | "ENDED" | "FAILED" | "UNKNOWN";
...
export type HqCallObservability = {
  ...
  /** Evidence available in the call row; never treated as proof of cross-channel persistence. */
  memoryEvidence: "SUMMARY READY" | "NO SUMMARY" | "INCOMPLETE";
...
export function buildCallObservability(c: HqCallSessionRow): HqCallObservability {
  ...
  if (sourceStatus === "failed" || sourceStatus === "missed") operationalState = "FAILED";
  else if (sourceStatus === "terminated" || c.ended_at) operationalState = "ENDED";
...
export function buildChannelActivity(input: {
  ...
  for (const c of input.calls) {
    const conversation = c.conversation_id ? (convById.get(c.conversation_id) ?? null) : null;
    const resolvedLeadId = c.lead_id ?? conversation?.lead_id ?? null;
    const lead = resolvedLeadId ? (leadById.get(resolvedLeadId) ?? null) : null;
    const attributedCall = resolvedLeadId === c.lead_id ? c : { ...c, lead_id: resolvedLeadId };
    ...
    items.push({
      ...
      sourceId: c.id,
      callObservability: buildCallObservability(attributedCall),
    });
  }
