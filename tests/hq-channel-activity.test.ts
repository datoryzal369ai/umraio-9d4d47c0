...
const messages = [
  { id: "m1", agency_id: "a1", conversation_id: "c1", sender: "customer", modality: "text", delivery_status: "sent", created_at: "2026-09-01T10:00:00Z" },
  { id: "m2", agency_id: "a1", conversation_id: "c1", sender: "customer", modality: "audio", delivery_status: "sent", created_at: "2026-09-01T11:00:00Z" },
  { id: "m3", agency_id: "a1", conversation_id: "c2", sender: "human", modality: "text", delivery_status: "failed", created_at: "2026-09-01T09:00:00Z" },
  { id: "m4", agency_id: "a1", conversation_id: "c1", sender: "customer", modality: "image", delivery_status: "sent", created_at: "2026-09-01T08:00:00Z" },
];

const calls = [
  { id: "k1", agency_id: "a1", lead_id: "l1", caller_phone: "+60123456789", direction: "inbound", status: "terminated", termination_reason: "completed", received_at: "2026-09-01T12:00:00Z", answered_at: "2026-09-01T12:00:05Z", ended_at: "2026-09-01T12:01:00Z", turn_count: 3 },
  { id: "k2", agency_id: "zz", lead_id: null, caller_phone: null, direction: "inbound", status: "failed", termination_reason: "gateway_http_502", received_at: "2026-09-01T07:00:00Z", answered_at: null, ended_at: null, turn_count: 0 },
];
...
describe("HQ channel normalization", () => {
  ...
  it("derives call status from lifecycle evidence", () => {
    expect(normalizeCallStatus({ status: "terminated", termination_reason: "completed", answered_at: "x" })).toBe("SUCCESS");
    expect(normalizeCallStatus({ status: "terminated", termination_reason: "completed", answered_at: null })).toBe("PARTIAL");
    expect(normalizeCallStatus({ status: "terminated", termination_reason: "session_timeout", answered_at: "x" })).toBe("PARTIAL");
    expect(normalizeCallStatus({ status: "failed", termination_reason: "failed", answered_at: null })).toBe("FAILED");
    expect(normalizeCallStatus({ status: "weird", termination_reason: null, answered_at: null })).toBe("UNKNOWN");
  });
...
describe("HQ channel activity feed", () => {
  ...
  it("never exposes message bodies and respects limits", () => {
    ...
    expect(buildChannelActivity({ messages, conversations, calls, leads, agencyNames, limit: 2 })).toHaveLength(2);
  });
